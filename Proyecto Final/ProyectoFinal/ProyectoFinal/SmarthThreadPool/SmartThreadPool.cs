...
namespace Amib.Threading
{
	...
	public partial class SmartThreadPool : WorkItemsGroupBase, IDisposable
	{
        ...
        //Obtener los datos del thread
        public List<NodoCola> ObtenerThreads()
        {
            List<NodoCola> lista = new List<NodoCola>();
            var PoolThreads = _workerThreads.Values;
            foreach (var thread in PoolThreads)
            {
                if (thread.CurrentWorkItem != null)
                {
                    lista.Add(thread.CurrentWorkItem.argumentos);
                }
            }
            return lista;
        }
		...
		public void Shutdown(bool forceAbort, int millisecondsTimeout)
		{
            ...
            Amib.Threading.Internal.Stopwatch stopwatch = Amib.Threading.Internal.Stopwatch.StartNew();
